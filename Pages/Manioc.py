...
dash.register_page(__name__, name="Manioc",path="/Manioc", order=1)
layout = dbc.Container([
    dbc.Row([
        html.H1('Detection de maladies pour la culture de Manioc',
    ...
    dbc.Row(dbc.Col([html.Div(id = 'bouton',
            children=dbc.Button(children='Predict \n',id="predict-button", color="primary", className="mt-3")),
            html.Br(),
            html.Div(id='prediction'),
             html.Br(),
...
def update(img,filename):
    ...
    return ''
@callback(
    Output("bouton", 'children'),
    Input('input_img', 'contents')
)

def update(contents):
    if contents:
        return dbc.Button(children='Predict',id="predict-button", color="primary", className="mt-3")
    else:
        return ""
@callback(
